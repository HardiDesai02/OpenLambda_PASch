...
class LeastLoadedScheduler:
    def __init__(self, workers):
        self.workers = workers  # List of worker nodes

    def schedule(self, image_name, current_time, required_libraries):
        # Select the worker with the least load
...
def simulate_scheduling(image_set, scheduler, num_processes=100, total_time=50):
    ...
    num_workers = len(scheduler.workers)  # Assuming the scheduler has this attribute
    ...
    while current_time < total_time and len(process_list) < num_processes:
        ...
        for image in current_arrivals:
            ...
            # Schedule the image using the scheduler
...
# Simulate scheduling with the Least-Loaded Scheduler
...
df.to_excel("image_least_loaded_worker_nodes.xlsx", index=False)
wb = openpyxl.load_workbook("image_least_loaded_worker_nodes.xlsx")
ws = wb.active

# Insert thumbnails of images into Excel (increased size)
...
wb.save("image_least_loaded_worker_nodes_with_images.xlsx")

# Download the result file
files.download("image_least_loaded_worker_nodes_with_images.xlsx")
